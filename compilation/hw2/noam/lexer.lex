...
<COMMENT_COND><<EOF>>			                    		errorHandler(UNCLOSED_COMMENT);								
<COMMENT_COND>.		                    					{printf("Error /\n");exit(0);}	
(({letter}|-{letter}){valid_name_char}*)			 		{return NAME;}
#(({digit}|{letter}|-{letter}){valid_name_char}*)	 		{return HASHID;}
@import 													{return IMPORT;}
\!{whitespace}*[iI][mM][pP][oO][rR][tT][aA][nN][tT]			{return IMPORTANT;}
{tick_1}													{ BEGIN(STRING_1); beginString1();}
<STRING_1>({printable_char_for_tick_1})* 	  				handleString1();
<STRING_1>(\\t)												handleTab();
<STRING_1>(\\r)												handleR();
<STRING_1>(\\n)												handleNewLine();
<STRING_1>(\\\\)											handleBackSlash();
...
<HEX_COND_1>{printable_char}								errorHandler(UNDEFINED_ESCAPE_SEQUENCE);
<HEX_COND_1>.												{printf("Error \"\n");exit(0);}
<STRING_1>{tick_1}											{BEGIN(INITIAL); return STRING;}
<STRING_1><<EOF>>			                    			errorHandler(UNCLOSED_STRING);								
<STRING_1>.													{printf("Error \'\n");exit(0);}	
...
<HEX_COND_2>{printable_char}								errorHandler(UNDEFINED_ESCAPE_SEQUENCE);
<HEX_COND_2>.												{printf("Error \"\n");exit(0);}
<STRING_2>{tick_2}											{BEGIN(INITIAL); return STRING;}
<STRING_2><<EOF>>			                    			errorHandler(UNCLOSED_STRING);								
<STRING_2>.													{printf("Error \"\n");exit(0);}	
[\>\+\~]													{return COMB;}
:															{return COLON;}
;															{return SEMICOLON;}
\{															{return LBRACE;}
...
\*															{return ASTERISK;}
\.															{return DOT;}
((\-)|(\+))?((0x({hex_digit})+)|({digit})+)					{return NUMBER;}
{digit}*(\.)?{digit}+(%|[a-z]+)								{return UNIT;}
rgb\(                                                       BEGIN(RGB_COND);
